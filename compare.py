...
NOTE - tweaked for FDI. Will automatically try and run afgainst each of the expected files
...
# ====
for x in range(1,11):
    dataset = "Transform_Dataset" + str(x)

    try:
        compare(dataset, dataset, '.csv') # we know the names will always match
    except:
        print ''
        print 'Could not validate ' + dataset
        print ''
